...
def _profile_secret_state(registry: ProviderRegistry, alias: str, fallback: str = "") -> str:
    profile = registry.profiles.get(alias)
    return _masked_secret_state(profile.api_key() if profile is not None else fallback)
