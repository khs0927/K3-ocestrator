...
| DS2API Go agent | DeepSeek-only upstream and Secret-file boundary | `ds2api-multi-provider` PR #2, head `291ed32` | local Go test/vet and remote Quality Gates run `31279315631` pass |
| K3 Python agent | ACP provider routing, DS2API fallback, K3 API/self-hosted profiles, MCP/API tools, stream metadata | implementation head `b8ea968` | complete for offline gate |
