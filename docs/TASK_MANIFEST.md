...
| DS2API Go agent | DeepSeek-only upstream and Secret-file boundary | `ds2api-multi-provider` PR #1/#2 | code pushed; remote Go check unavailable |
| K3 Python agent | ACP provider routing, DS2API fallback, MCP/API tools, stream metadata | commit `7e2fa6f` | complete for offline gate |
